...
const renderCard = (id, url, name, likes, element) => {
  ...
  card.innerHTML = `
    <div class="card movie-card" id=${id} data-id=${id}>
        ...
        <div class="d-flex justify-content-between">
          ...
          <div class="heart">
            <div class="d-flex">
              ...
              <div><span class="like-count mb-4">${likes}${
    likes === 1 ? 'Like' : 'Likes'
  }</span></div>
            </div>
          </div>
        </div>
        <a data-id=${id} href="#" class="btn btn-outline-primary mt-2 comments-btn" data-toggle="modal" data-target="#movieModal">Comments</a>
      </div>
    </div>
  `;
...
export const populateView = async (data, element) => {
  ...
  for (let i = 0; i < data.length; i += 1) {
    const { id, url, name, likes } = data[i];
